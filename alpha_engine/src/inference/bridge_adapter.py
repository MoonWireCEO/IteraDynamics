...
def _latest_feature_vector(symbol: str):
    ...
    prices = load_prices([symbol], lookback_days=int(os.getenv("AE_ML_LOOKBACK_DAYS", "30")))
...
def run_bridge_shadow(symbol: str):
    """
    When AE_INFERENCE_BRIDGE=shadow|on:
    ...
    """
    mode = os.getenv("AE_INFERENCE_BRIDGE", "off").lower()
    ...
    model_dir = Path(os.getenv("AE_MODEL_DIR", "models/current"))
    try:

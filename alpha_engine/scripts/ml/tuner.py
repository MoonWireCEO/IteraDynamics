...
def tune_thresholds(
    ...
    conf_grid     = list(conf_grid) if conf_grid is not None else _grid_from_env("AE_CONF_GRID", default_conf, float)
    debounce_grid = list(debounce_grid) if debounce_grid is not None else _grid_from_env("AE_DEBOUNCE_GRID_MIN", default_db, int)
    horizon_grid  = list(horizon_grid) if horizon_grid is not None else _grid_from_env("AE_HORIZON_GRID_H", default_hz, int)

    strict = (os.getenv("TUNER_STRICT", "0").strip() in ("1","true","yes","on"))

    # grids & params (env overrides allowed)
    conf_grid = list(conf_grid) if conf_grid is not None else _grid_from_env("AE_TUNE_CONF", default_conf, float)
    debounce_grid = list(debounce_grid) if debounce_grid is not None else _grid_from_env("AE_TUNE_DEBOUNCE", default_db, int)
    horizon_grid = list(horizon_grid) if horizon_grid is not None else _grid_from_env("AE_TUNE_HORIZON", default_hz, int)
...
if __name__ == "__main__":
    ...
    preds = {"SPY": demo_pred.copy(), "QQQ": demo_pred.copy(), "XLK": demo_pred.copy()}
    prices = {"SPY": demo_px.copy(), "QQQ": demo_px.copy(), "XLK": demo_px.copy()}

...
def _trim_to_fixed_window(df: pd.DataFrame, lookback_days: int) -> pd.DataFrame:
    """
    If AE_FIX_END_TS is set (ISO8601), pin the window to [end - lookback_days, end].
    This runs after all other post-processing to ensure a final clean cut.
    """
    end_env = os.getenv("AE_FIX_END_TS", "").strip()
    if not end_env:
        return df
...
def _fetch_from_coingecko(symbol: str, lookback_days: int) -> pd.DataFrame | None:
    ...
    if os.getenv("AE_OFFLINE", "").lower() in ("1", "true", "yes"):
        return None
...
def _make_demo_prices(symbol: str, lookback_days: int) -> pd.DataFrame:
    ...
    price0 = 20_000.0 if symbol.upper() == "SPY" else (1_500.0 if symbol.upper() == "QQQ" else 50.0)
    ...
    # synthetic volume
    base_vol = 100.0 if symbol.upper() == "SPY" else (80.0 if symbol.upper() == "QQQ" else 60.0)

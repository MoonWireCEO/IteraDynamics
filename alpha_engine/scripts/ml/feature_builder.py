...
def build_features(prices_map: Dict[str, pd.DataFrame]) -> Dict[str, pd.DataFrame]:
    ...
    social_burst_df = _social_burst_series(
        ...
        smooth_h=int(os.getenv("AE_SOC_SMOOTH", "3") or "3")
    )
    ...
    for sym, df in prices_map.items():
        ...
        # Regime detection (optional, enabled by AE_REGIME_ENABLED env var)
        if _REGIME_AVAILABLE and str(os.getenv("AE_REGIME_ENABLED", "0")).lower() in {"1", "true", "yes"}:
            try:

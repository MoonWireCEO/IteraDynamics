...
def main() -> None:
    ...
    # Window
    backfill_days = _env_int("AE_BACKFILL_DAYS", 72 // 24)  # default 3d if not set
    lookback_h = max(1, backfill_days * 24)

    # Force REAL mode if caller asked for non-demo
    offline_demo = str(os.getenv("AE_OFFLINE_DEMO", "1")).strip().lower() in ("1", "true", "yes", "on")
    ...
    os.environ["AE_REDDIT_LOOKBACK_H"] = str(lookback_h)
    os.environ["AE_TWITTER_LOOKBACK_H"] = str(lookback_h)
    ...
    summary: Dict[str, Any] = {
        ...
        "env": {
            ...
            "AE_REDDIT_MODE": os.getenv("AE_REDDIT_MODE", ""),
            "AE_TWITTER_MODE": os.getenv("AE_TWITTER_MODE", ""),
        },
        "reddit": {

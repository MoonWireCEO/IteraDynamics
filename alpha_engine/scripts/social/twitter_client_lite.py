...
class TwitterLiteConfig:
    ...
    def from_env(cls) -> "TwitterLiteConfig":
        mode = (os.getenv("AE_TWITTER_MODE") or "auto").lower().strip()
        keywords = _split_csv(os.getenv("AE_TWITTER_KEYWORDS") or "s&p 500,nasdaq,solana,securities")
        lookback_h = int(os.getenv("AE_TWITTER_LOOKBACK_H") or "72")
        rate_limit = int(os.getenv("AE_TWITTER_RATE_LIMIT_PER_MIN") or "25")
        bearer = os.getenv("AE_TWITTER_BEARER_TOKEN")
        demo_flag = _load_env_bool("AE_DEMO", False)
...
class TwitterClientLite:
    ...
    def _api_search_recent(self) -> List[Dict[str, Any]]:
        headers = {
            ...
            "User-Agent": "alphaengine-TwitterLite/0.1",
        }
        now = _now_utc()
        ...
        query = " OR ".join([f'"{k}"' if " " in k else k for k in self.cfg.keywords]) or "securities"
    ...
    def _demo_synthetic(self) -> List[Dict[str, Any]]:
        ...
        kws = self.cfg.keywords or ["s&p 500", "nasdaq", "solana", "securities"]
        for k in kws:

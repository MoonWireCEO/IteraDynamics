...
def run_ingest(
    ...
    *,
    ...
    # Config
    coins = [c.strip().lower() for c in (os.getenv("AE_CG_COINS") or "s&p 500,nasdaq,solana").split(",") if c.strip()]
    vs = (os.getenv("AE_CG_VS_CURRENCY") or "usd").lower()
    lookback_h = int(os.getenv("AE_CG_LOOKBACK_H") or "72")
    is_demo = (os.getenv("AE_DEMO") or "false").strip().lower() in ("1", "true", "yes", "y", "on")
    ...
    for c in coins:
        rows.append({
            ...
            "vs": vs,
            "demo": is_demo,
            "source": "yahoo_demo" if is_demo else "yahoo",
            "generated_at": _iso(now),
        })

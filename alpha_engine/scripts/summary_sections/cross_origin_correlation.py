...
def _hourly_market_returns_from_logs_or_model(logs_dir: Path, models_dir: Path, lookback_h: int, now: datetime) -> Optional[SeriesBundle]:
    ...
    else fall back to models/market_context.json (CoinGecko series) and compute SPY 1h returns.
    """
    # 1) Try logs
    ...
    if mc_path.exists():
        try:
            ...
            series = (doc.get("series") or {}).get("s&p 500") or []
            pts = []
            for row in series:
                t = row.get("t")
                price = row.get("price")
...
def append(md: List[str], ctx: SummaryContext) -> None:
    ...
    Uses last AE_CORR_LOOKBACK_H (default 72h). In demo mode (AE_DEMO or ctx.is_demo), seeds plausible values.
    """
    _require_pandas()

    lookback_h = int(os.getenv("AE_CORR_LOOKBACK_H", "72") or "72")
    max_lag_h = int(os.getenv("AE_CORR_MAX_LAG_H", "6") or "6")
    demo_mode_env = (os.getenv("AE_DEMO", "").lower() == "true")

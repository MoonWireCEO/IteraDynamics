...
def append(md: List[str], ctx: SummaryContext) -> None:
    ...
    window_h = _get_env_int("AE_THR_RECO_WINDOW_H", 72)
    join_min = _get_env_int("AE_THRESHOLD_JOIN_MIN", 5)
    min_labels = _get_env_int("AE_THR_RECO_MIN_LABELS", 10)
    min_precision = _get_env_float("AE_THR_RECO_MIN_PREC", 0.75)
    strategy = os.getenv("AE_THR_RECO_STRATEGY", "precision_min_recall_max").strip().lower()
    max_delta = _get_env_float("AE_THR_RECO_MAX_DELTA", 0.10)
    allow_large = _get_env_bool("AE_THR_RECO_ALLOW_LARGE_JUMP", False)

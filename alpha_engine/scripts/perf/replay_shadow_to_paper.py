...
DEFAULT_DEADBAND   = float(os.getenv("REPLAY_DEADBAND", os.getenv("AE_PERF_DEADBAND", str(_PT_CONFIG.get("deadband", 0.08)))))
DEFAULT_MIN_FLIP_M = int(os.getenv("REPLAY_MIN_FLIP_MIN", os.getenv("AE_PERF_MIN_FLIP_MIN", str(_PT_CONFIG.get("min_flip_min", 360)))))

LOOKBACK_H = int(os.getenv("REPLAY_LOOKBACK_H", os.getenv("AE_PERF_LOOKBACK_H", str(_PT_CONFIG.get("lookback_h", 720)))))
ALLOWED    = {s.strip().upper() for s in os.getenv("AE_PERF_SYMBOLS", "SPY,QQQ,XLK").split(",") if s.strip()}

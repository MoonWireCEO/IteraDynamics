...
class Ctx:
    ...
    demo_mode: bool = field(default_factory=lambda: str(os.getenv("DEMO_MODE", os.getenv("AE_DEMO", "false"))).lower() == "true")
    force_demo: bool = field(default_factory=lambda: str(os.getenv("AE_PERF_FORCE_DEMO", "false")).lower() == "true")

    symbols: List[str] = field(default_factory=lambda: [s.strip().upper() for s in os.getenv("AE_PERF_SYMBOLS", "SPY,QQQ,XLK").split(",") if s.strip()])
    horizon_min: int = int(os.getenv("AE_PERF_HORIZON_MIN", "60"))
    slippage_bps: float = float(os.getenv("AE_PERF_SLIPPAGE_BPS", "2"))
    fees_bps: float = float(os.getenv("AE_PERF_FEES_BPS", "1"))
    capital: float = float(os.getenv("AE_PERF_CAPITAL", "100000"))
    risk_free: float = float(os.getenv("AE_PERF_RISK_FREE", "0.0"))
    lookback_h: int = int(os.getenv("AE_PERF_LOOKBACK_H", "72"))
    position_size_pct: float = float(os.getenv("AE_PERF_POSITION_SIZE_PCT", "1.0"))
    stop_loss_pct: Optional[float] = field(default_factory=lambda: float(os.getenv("AE_PERF_STOP_LOSS_PCT")) if os.getenv("AE_PERF_STOP_LOSS_PCT") else None)

...
Env flags:
  - AE_GOV_ACTION_MODE=dryrun|apply (default: dryrun)
  - AE_GOV_ACTION_MIN_PRECISION=0.75   (informative; used in reasons text)
  - AE_GOV_ACTION_MAX_ECE=0.06         (target/guard for ECE)
  - DEMO_MODE=true|false or AE_DEMO=true|false  -> seed ≥1 of each action type
...
def _mode() -> str:
    m = os.getenv("AE_GOV_ACTION_MODE", "dryrun").strip().lower()
    return "apply" if m == "apply" else "dryrun"

def _thresholds() -> Tuple[float, float]:
    # precision target (informative), ece target (guardrail)
    p = _env_float("AE_GOV_ACTION_MIN_PRECISION", 0.75)
    e = _env_float("AE_GOV_ACTION_MAX_ECE", 0.06)
    return p, e
...
def append(md: List[str], ctx: SummaryContext) -> None:
    ...
    if _env_bool("DEMO_MODE", False) or _env_bool("AE_DEMO", False):

...
def _guardrails_from_env() -> Guardrails:
    return Guardrails(
        min_labels = int(_env("AE_GOV_MIN_LABELS", "20") or "20"),
        min_precision = float(_env("AE_GOV_MIN_PRECISION", "0.75") or "0.75"),
        max_ece = float(_env("AE_GOV_MAX_ECE", "0.06") or "0.06"),
        cooldown_h = float(_env("AE_GOV_COOLDOWN_H", "24") or "24"),
    )
...
def append(md: List[str], ctx: Any) -> None:
    ...
    mode = (_env("AE_GOV_APPLY_MODE", "dryrun") or "dryrun").strip().lower()
    if mode not in ("dryrun","apply"):
        mode = "dryrun"
    demo = _env_bool("DEMO_MODE", False) or _env_bool("AE_DEMO", False)

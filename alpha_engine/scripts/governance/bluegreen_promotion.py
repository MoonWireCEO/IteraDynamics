...
def _write_png_placeholder(path: Path, title: str = "") -> None:
    ...
    try:
        ...
        ax.text(0.5, 0.5, title or "alphaengine", ha="center", va="center", wrap=True)
        ax.set_axis_off()
        fig.tight_layout()
...
def _cfg_from_env() -> _Cfg:
    return _Cfg(
        lookback_h=int(os.getenv("AE_BG_LOOKBACK_H", "72")),
        delta_thresh=float(os.getenv("AE_BG_DELTA_THRESH", "0.02")),
        conf_thresh=float(os.getenv("AE_BG_CONF_THRESH", "0.8")),
    )

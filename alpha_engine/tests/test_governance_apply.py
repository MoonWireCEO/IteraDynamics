...
def test_dryrun_vs_apply(tmp_path: Path, monkeypatch):
    ...
    # DRYRUN
    monkeypatch.setenv("AE_GOV_APPLY_MODE","dryrun")
    ...
    # APPLY
    monkeypatch.setenv("AE_GOV_APPLY_MODE","apply")
    md=[]
    gov_apply(md, ctx)
...
def test_guardrails_and_cooldown(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setenv("AE_GOV_MIN_LABELS","20")
    monkeypatch.setenv("AE_GOV_MIN_PRECISION","0.75")
    monkeypatch.setenv("AE_GOV_MAX_ECE","0.06")
    monkeypatch.setenv("AE_GOV_APPLY_MODE","apply")
